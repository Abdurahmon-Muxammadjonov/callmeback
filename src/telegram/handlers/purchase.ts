import { TARIFFS, formatSum, monthlyTotal, baseMonthlyTotal, periodTotal } from '../pricing';
import {
...
import { insertLead, logEvent } from '../leadsDb';

const PHONE_REGEX = /^\+?\d[\d\s\-()]{6,}$/;

export async function startPurchaseFlow(ctx: SessionContext): Promise<void> {
  await resetSession(ctx);
  await logEvent(ctx.from!.id, 'purchase_flow_started');
  await ctx.reply('Qaysi tarifni tanlaysiz?', tariffInlineKeyboard());
}

export async function onTariffSelected(ctx: SessionContext & { match: RegExpExecArray }): Promise<void> {
...
export async function onTariffConfirm(ctx: SessionContext & { match: RegExpExecArray }): Promise<void> {
...
export async function handleEmployeeCount(ctx: SessionContext, text: string): Promise<void> {
  ...
  const tariffKey = ctx.session.tariffKey as string;
  ...
  await ctx.reply(lines.join('\n'), { parse_mode: 'Markdown', ...durationInlineKeyboard(tariffKey, n) });
}

export async function onDurationSelected(ctx: SessionContext & { match: RegExpExecArray }): Promise<void> {
...
export async function onFinalConfirm(ctx: SessionContext & { match: RegExpExecArray }): Promise<void> {
  const choice = ctx.match[1];
  const { tariffKey, employeeCount } = ctx.session;

  if (choice === 'no') {
    await ctx.answerCbQuery();
    await ctx.editMessageText('Muddatni qayta tanlang:', durationInlineKeyboard(tariffKey as string, employeeCount as number));
    return;
  }
...
export async function handleName(ctx: SessionContext, text: string): Promise<void> {
  ...
  await ctx.reply('Telefon raqamingizni yuboring (tugma orqali eng ishonchli):', PHONE_REQUEST_KEYBOARD);
}

export async function handlePhoneContact(ctx: SessionContext): Promise<void> {
...
export async function handlePhoneText(ctx: SessionContext, text: string): Promise<void> {
...
async function proceedToCompany(ctx: SessionContext, phone: string): Promise<void> {
  ctx.session.phone = phone;
  ctx.session.state = 'awaiting_company';
  await ctx.reply('Kompaniyangiz nomini kiriting:', { remove_keyboard: true } as any);
}

export async function handleCompany(ctx: SessionContext, text: string): Promise<void> {
  const company = text.trim();
  if (!company) { await ctx.reply('Iltimos, kompaniya nomini kiriting.'); return; }
  ctx.session.company = company;
  await finalizeLead(ctx);
}

async function finalizeLead(ctx: SessionContext): Promise<void> {
  const s = ctx.session;
  const tariffKey = s.tariffKey as string;
  const duration = s.duration as number;
  const employeeCount = s.employeeCount as number;
  const perEmployee = monthlyTotal(tariffKey, duration, 1);
  const monthly = monthlyTotal(tariffKey, duration, employeeCount);
  const total = periodTotal(tariffKey, duration, employeeCount);

  const lead = {
    telegram_user_id: ctx.from!.id,
    telegram_username: (ctx.from as any)?.username || null,
    full_name: s.fullName as string,
    phone: s.phone as string,
    company_name: s.company as string,
    tariff: TARIFFS[tariffKey].name,
  ...
  await resetSession(ctx);
  await ctx.reply("So'rovingiz qabul qilindi. Tez orada operatorlarimiz siz bilan bog'lanadi.", MAIN_MENU);
  ...
  const destinations = [process.env.ADMIN_CHAT_ID, process.env.GROUP_CHAT_ID].filter(Boolean) as string[];
